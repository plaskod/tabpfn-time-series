#!/usr/bin/env python3
"""
Modified evaluate.py script with native multi-GPU support
"""
...
from gift_eval.dataset_definition import (
    MED_LONG_DATASETS, ALL_DATASETS, DATASET_PROPERTIES_MAP,
)

# Import the multi-GPU enabled wrapper
from tabpfn_ts_wrapper_multigpu import TabPFNTSPredictor
...
def append_results_to_csv(
    res, csv_file_path, dataset_metadata, model_name,
):
...
def log_results_to_wandb(res, dataset_metadata):
    wandb_log_data = {
...
def main(args):
    ...
    for i, (sub_dataset, dataset_metadata) in enumerate(sub_datasets):
        ...
        # Use the multi-GPU enabled predictor
        tabpfn_predictor = TabPFNTSPredictor(
            ds_prediction_length=sub_dataset.prediction_length,
            ds_freq=sub_dataset.freq,
            tabpfn_mode=TabPFNMode.LOCAL,  # or TabPFNMode.LOCAL
            context_length=4096,
            debug=args.debug,
            num_workers=args.num_workers,  # Use specified number of workers
            gpu_ids=args.gpu_ids,  # Use specified GPUs
        )
        ...
        log_results_to_wandb(res=res, dataset_metadata=dataset_metadata)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int, 
        default=None,
        help="Number of workers for parallel processing (default: auto-detect GPUs)"
    )
    parser.add_argument(
        "--gpu_ids",
        type=str,
        default=None,
        help="Comma-separated GPU IDs to use (e.g., '0,1,2,4')"
    )
